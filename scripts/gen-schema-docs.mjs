...
import { ROOT, loadSchemas, emit, pageTitle } from "./lib.mjs";
...
const PAGES = {
  ...
  "frame-inline": { title: "frame-inline.schema.json", description: "Canonical schema for obs.frame / cmd.frame params in the inline binding.", spec: "/spec/transport/frames" },
  ...
  "session-telemetry": { title: "session-telemetry.schema.json", description: "Canonical schema for session.telemetry notifications.", spec: "/spec/loop/time-models" },
  "obs-report": { title: "obs-report.schema.json", description: "Canonical schema for obs.report receiver reports.", spec: "/spec/loop/observations" },
  "ping": { title: "ping.schema.json", description: "Canonical schema for ping params; the result is ping-result.schema.json.", spec: "/spec/semantics/timestamps-and-clocks" },
  "session-state": { title: "session-state.schema.json", description: "Canonical schema for session.state notifications.", spec: "/spec/session/lifecycle" },
  "tick": { title: "tick.schema.json", description: "Canonical schema for world.tick params; the result is tick-result.schema.json.", spec: "/spec/loop/time-models" },
  "subscribe": { title: "subscribe.schema.json", description: "Canonical schema for obs.subscribe params; obs.unsubscribe uses unsubscribe.schema.json and both return subscribe-result.schema.json.", spec: "/spec/session/negotiation" },
  "reset": { title: "reset.schema.json", description: "Canonical schema for world.reset params; world.restore uses restore.schema.json and both return reset-result.schema.json.", spec: "/spec/safety/permissions" },
  "action-ref": { title: "action-ref.schema.json", description: "Canonical schema for action.cancel params and the action.status pull request.", spec: "/spec/loop/action-lifecycle" },
...
for (const [name, meta] of Object.entries(PAGES)) {
  ...
  body += `<Note>Generated from [\`schemas/v0.1/${name}.schema.json\`](${src}) (\`$id\`: \`${schema.$id}\`). Normative text: [${pageTitle(meta.spec)}](${meta.spec}).</Note>\n\n`;

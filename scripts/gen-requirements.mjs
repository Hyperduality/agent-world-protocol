...
import { ROOT, walk, rel, emit, pageTitle } from "./lib.mjs";
...
<Note>
${rows.size} requirements: ${counts.world} world-side, ${counts.agent} agent-side, ${counts.both} both. ${counts.MUST} MUST, ${counts.SHOULD} SHOULD, ${counts.MAY} MAY. ${counts.manual} require manual evidence; ${counts.untestable} are process or deployment policy and are not tested by the suite. Every bracketed ID in the specification has a row here, and every row names an ID the specification defines.
</Note>

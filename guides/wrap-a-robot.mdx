...
<Steps>
  ...
  <Step title="Wire the e-stop both ways">
    Hardware e-stop → `world.event: e_stop_engaged`; `executing` actions end `failed` and pre-execution actions `cancelled`, both with reason `e_stop`, and submissions fail with `AWP_ESTOP_ACTIVE` until `e_stop_released` (AWP-EVT-002). Expose a `stop` action type with `preemption: replace` as the software path.
  </Step>

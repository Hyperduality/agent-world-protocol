...
- `session.ready` declares `heartbeat_interval_ms`. Each side MUST send `ping` at least every `heartbeat_interval_ms` and MUST answer `ping` with `pong` within `heartbeat_interval_ms`. `[AWP-SAF-001]`

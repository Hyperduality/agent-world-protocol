...
Some control-plane methods change state that other sessions observe or depend on. They form a fourth grant class, `admin`, requested in `session.open` (`admin: string[]`) and enumerated in `session.ready` (`granted.admin`):

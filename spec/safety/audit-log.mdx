...
Worlds MUST record, per session, an append-only log of: manifests, granted subset, every control-plane message (both directions, after [redaction](#redaction)), every action status transition, every safety event and approval, and for each data-plane frame in either direction either the payload or its SHA-256 hash plus header. `[AWP-AUD-001]`

...
Stream bindings beyond `inline`, snapshots, approval, envelopes, and the measured watchdog test (AWP-SAF-012) are **not** required by Core; they arrive with profiles. A Core world that offers streaming does declare `safe_state` and run its watchdog (AWP-MAN-006, AWP-SAF-003..008), stamps `ts_send_ns` on every observation frame (AWP-OBS-006), and sends `session.telemetry` (AWP-TIM-006), because those are part of the streaming time model itself.

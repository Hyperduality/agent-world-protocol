...
- **Resumption.** On `session.resume` the control channel replays per AWP-CTL-008; the agent re-establishes stream connections from the re-issued `stream_endpoints`; on every `reliable` channel the world's first frame after resumption MUST be a keyframe carrying the `resync` flag, and the `seq` gap it reveals is not a loss (AWP-DAT-009); `latest-wins` channels continue unchanged, except that in lockstep every per-tick channel restarts with a resync keyframe (AWP-TIM-009). A world closes a session's stream connections when the session is suspended; until the agent re-establishes them its channels are delivered inline (AWP-TRN-012). `[AWP-TRN-008]`

## Bindings

- The control channel is carried on one **control connection** per session (AWP-CTL-007); worlds MUST offer the WebSocket binding for it, on which the connection uses the `awp` subprotocol and each JSON-RPC message is one text message. Optional control bindings: stdio (local child processes) and gRPC. `[AWP-TRN-001]`
...
- **Stream reconnection.** If a stream connection is lost while the control connection remains, the agent MAY reconnect to the same endpoint with the session token at any time and the world MUST accept it; grants are unaffected, `reliable` channels restart with a resync keyframe (AWP-DAT-009), and the world reports the outage on affected `reliable` channels through `channel_degraded` (AWP-SAF-009). Until it reconnects, the channels that connection carried are not delivered at all, in either direction — not inline — so an agent that lost a stream learns so from the event rather than from a silent change of path. `[AWP-TRN-010]`
- **Moving a channel.** A channel is delivered inline until a stream connection that carries it is established, and from then on only on that connection. An endpoint that omits `channels` carries every channel of the session, observation and command, including channels subscribed later. The world's first frame of each observation channel on a newly established stream connection MUST be a keyframe carrying the `resync` flag. Because order across connections is not preserved (AWP-TRN-007), an inline frame may arrive after a newer frame of its channel on the stream connection; a receiver MUST discard a frame whose `seq` is not greater than the last it processed on that channel when the two arrived on different connections, and MUST NOT count it as loss or report it as a violation. `[AWP-TRN-012]`
- **Stream connections.** A stream connection is opened against an endpoint URL with the session token as its bearer credential (AWP-SEC-004..006) and the `awp` subprotocol. On the `ws` binding every WebSocket message is binary and carries exactly one frame in the [binary envelope](/spec/transport/frames), world→agent for observation channels and agent→world for command channels; text messages MUST NOT be sent, and a receiver MUST close a stream connection on which a message is not a valid frame (AWP-DAT-010), with WebSocket close code 1002 and reason `AWP_MALFORMED`. A token that names no session with a live control connection fails the stream connection. `[AWP-TRN-013]`

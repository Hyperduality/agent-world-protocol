...
- **Resumption.** On `session.resume` the control channel replays per AWP-CTL-008; the agent re-establishes stream connections from the re-issued `stream_endpoints`; on every `reliable` channel the world's first frame after resumption MUST be a keyframe carrying the `resync` flag, and the `seq` gap it reveals is not a loss (AWP-DAT-009); `latest-wins` channels continue unchanged. A world closes a session's stream connections when the session is suspended; until the agent re-establishes them its channels are delivered inline (AWP-TRN-012). `[AWP-TRN-008]`

...
A manifest fragment declaring one command channel and its anchoring action (full example: [`examples/v0.1/world-manifest-servo.json`](https://github.com/Hyperduality/agent-world-protocol/blob/main/examples/v0.1/world-manifest-servo.json)):
...
- Every action schema naming a `command_channel` MUST declare `duration: "streaming"` and reference a declared command channel; a granted streaming action type implies access to its bound channel, which `session.ready` lists in `granted.channels` with its `channel_id` and rate like any other channel. `[AWP-CMD-002]`

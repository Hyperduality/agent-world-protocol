...
description: "Agent→world data-plane streams for high-rate setpoints, bound to a streaming action."
---

The data plane is symmetric. Worlds MAY declare **command channels** — writable channels carrying agent→world frames — for control that runs faster than per-submission JSON-RPC is suited to: torque- and servo-level setpoints at 100–1000 Hz. Action chunking at 10–50 Hz does not need them; use the chunk-replacement idiom in [Actions](/spec/loop/actions).
...
- Frames carry the standard [data-plane envelope](/spec/transport/data-plane) with per-channel monotonically increasing `seq`; worlds MUST apply frames in `seq` order and, on `latest-wins` channels, MUST drop a frame older than the last applied one. `[AWP-CMD-007]`

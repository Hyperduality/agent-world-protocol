...
Worlds emit `world.event` notifications for state changes not tied to a specific action: `entity_appeared`, `entity_removed`, `collision`, `e_stop_engaged`, `e_stop_released`, `envelope_violation`, `grant_expired`, `safe_state_entered`, `safe_state_exited`, `data_plane_degraded`, `world_resetting`, `world_shutdown`. Events carry `ts_mono_ns`, `status_seq`, and structured detail. Custom events use `x-<vendor>.` names. `[AWP-EVT-001]`
...
| 2xxx | Session | `AWP_EMBODIMENT_UNAVAILABLE` (2001), `AWP_TIME_MODEL_UNSUPPORTED` (2002), `AWP_SESSION_EXPIRED` (2003), `AWP_SESSION_EXISTS` (2004), `AWP_SESSION_UNKNOWN` (2005), `AWP_INTEGER_RANGE` (2006) |
| 3xxx | Action | `AWP_PARAMS_INVALID` (3001), `AWP_BUSY` (3002), `AWP_QUEUE_FULL` (3003), `AWP_ACTION_ID_CONFLICT` (3004), `AWP_ESTOP_ACTIVE` (3005), `AWP_TICK_NOT_AUTHORIZED` (3006) |

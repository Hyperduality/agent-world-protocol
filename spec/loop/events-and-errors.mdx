...
Terminal `action.status` notifications in `rejected`, `failed`, or `cancelled` carry a `reason`, and `preempted` MAY carry one, from this registry (AWP-LIF-004). Vendor reasons use `x-<vendor>.` names.
...
| `e_stop` | failed, cancelled | `e_stop_engaged` (AWP-EVT-002); `failed` from `executing` or `cancelling` |
| `connection_lost` | failed | Safe-state entry terminated execution or an abort (AWP-SAF-004) |
...
| `transferred` | cancelled, preempted | The embodiment was transferred to another session (AWP-EMB-003) |
| `abort_failed` | failed | The safe abort of a `cancelling` action could not complete, or exceeded `max_abort_ms` (AWP-LIF-010) |
...
Every AWP error object (codes 1000–4999) marks `retryable: true|false` in `data`. JSON-RPC reserved errors (−32768 to −32000, e.g. `-32601` for an unknown method, AWP-CTL-002) MAY omit `data` and are non-retryable unless `data` says otherwise. Agents MUST NOT retry a request refused with a non-retryable error with identical parameters; a submission retries a refused one when its `type`, `params`, and `embodiment_id` are the same, whatever its `action_id`. `[AWP-ERR-001]`

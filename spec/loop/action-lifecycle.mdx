...
The normative transition table, including the reasons each transition may carry, is [`spec/action-lifecycle.yaml`](https://github.com/Hyperduality/agent-world-protocol/blob/main/spec/action-lifecycle.yaml).

## States

| State | Class | Meaning |
|---|---|---|
| `submitted` | transient | The world has received the intent. Never reported on the wire. |
...
- Every transition MUST be one listed in the transition table. The world MUST report every transition after admission exactly once: a transition an `action.cancel` causes in its result, every other one as an `action.status` notification. Terminal states are `rejected`, `completed`, `failed`, `preempted`, `cancelled`. Every action reaches exactly one logical terminal state. `[AWP-LIF-001]`

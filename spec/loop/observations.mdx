...
- In lockstep, per-tick channels additionally carry `tick` (u64) in the frame's [extension block](/spec/transport/data-plane#extension-block-present-only-when-flags-bit-2-is-set) (type `0x01`) or as the `tick` field in the inline JSON form — never inside the payload (AWP-DAT-007). `[AWP-OBS-002]`

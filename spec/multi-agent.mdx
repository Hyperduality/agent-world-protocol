...
- Mutations that affect other sessions — `world.reset`, `world.restore`, and `world.tick` under `tick_authority: "any_session"` on a world that may bind several lockstep sessions — are administrative: they require the `admin` grant class (AWP-PRM-005) and their cross-session effects are specified in AWP-PRM-006 and AWP-TIM-012. Affected sessions learn of a reset or restore through `world.event: world_resetting` and of an advance through the advance itself (AWP-TIM-003). A `barrier` call is not administrative: it only records the caller's readiness. A world that hosts more than one lockstep session MUST declare `tick_authority: "barrier"` unless it grants `tick` to exactly one session. `[AWP-MA-005]`

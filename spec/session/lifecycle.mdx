...
- Heartbeat obligations begin at `session.ready` (AWP-SAF-001). A world MAY close a connection that has no session after a period of agent silence of its choosing, which SHOULD be at least 15 s. Neither side detects loss by silence before `session.ready`, or on a new connection before the `session.resume` result (AWP-SAF-002); an agent that holds such a connection open SHOULD send `ping` meanwhile, at least every 5 s; its result is stamped on a world clock that is not a session clock and MUST NOT enter the offset estimate (AWP-CLK-008). `[AWP-SES-012]`

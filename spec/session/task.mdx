...
The instruction that conditions a language-conditioned policy ("pick up the red block") is session state, not an action: it names what the agent is trying to do, it can change mid-session, and approvers and audit tooling need to see it. AWP gives it one standard home — `awp.task`.

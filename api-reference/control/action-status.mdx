...
<ParamField body="status_seq" type="integer" required>Per-session sequence shared by `action.status`, `world.event`, and `session.state`, and by the `action.submit` / `action.cancel` results. Unique within the session; receivers deduplicate on it (AWP-LIF-009) and pass the highest seen value to `session.resume` (AWP-CTL-008).</ParamField>
<ParamField body="ts_mono_ns" type="integer" required>Time of the transition in the world, on the session clock.</ParamField>
<ParamField body="tick" type="integer">Lockstep: the tick of the transition.</ParamField>

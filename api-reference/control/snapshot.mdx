...
**Errors:** 4001 (`AWP_FORBIDDEN`: no matching `admin` grant, AWP-PRM-005), 3001 (`AWP_PARAMS_INVALID`: `world.restore` with a token the world does not hold, AWP-REP-002). Without `capabilities.snapshot`, `world.snapshot` and `world.restore` are unknown methods (AWP-VER-007).

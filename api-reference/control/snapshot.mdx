...
**world.reset** — `{ initial_state?: string, seed?: integer }` → return to a named initial state (default: the manifest's first `initial_states` entry); same cross-session effects as restore, with `"kind": "reset"`. `seed` requires `capabilities.seed`. Result as for restore.

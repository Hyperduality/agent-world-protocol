...
Little-endian throughout. Flags: bit 0 keyframe, bit 1 end-of-burst, bit 2 has_extensions, bits 3–7 reserved (zero on send, ignored on receive). Extension types: `0x01` tick (u64), `0x02` ts_sim_ns (i64), `0x80`–`0xFF` vendor. `payload_len` counts payload bytes only.
...
`flags` in JSON carries bits 0–1 only; extension values are the named fields `tick` and `ts_sim_ns` (vendor extensions: `x-<vendor>.<name>`).

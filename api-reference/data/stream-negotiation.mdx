...
The agent connects to the first binding it supports; `inline` is always last and always available (AWP-TRN-004). Stream connections authenticate with the session token in the `Authorization` header or the `awp.bearer.<token>` WebSocket subprotocol, never in the URL (AWP-SEC-004..006), and are re-established from the re-issued endpoints after `session.resume` (AWP-TRN-008).

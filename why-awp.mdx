...
**One agent, sim and real.** The protocol defines two time models — lockstep for pausable worlds, streaming for reality. The schemas stay identical, so the same agent binary can train against a simulator world and deploy against a robot world ([`awp-sim`](/adapters/awp-sim) is the reference simulator world). See [Sim-to-real](/guides/sim-to-real).
...
AWP does not replace any of the standards above — it composes with them. An AWP world can expose MCP tools; an AWP robotics adapter can sit on ROS topics; agents inside an AWP world can talk A2A to each other. See the [Compare](/compare/mcp) section.
